...
import experiments.tooth.preprocessing as pp
import math
import gc
...
def get_test_generator(cf, logger):
    ...
    # test_paths = [f for f in test_paths if 'img.npy' in f]
    # test_paths = [f for f in test_paths for i in test_ix if str(i) in f]
    # print(test_paths)
    ...
    # print('test_pids_list = ', test_pids)
    
    logger.info("data set loaded with: {} test patients.".format(len(test_data.keys())))
...
def load_dataset(cf):
    ...
    raw_pids = p_df.raw_pid.tolist()
    imgs = [os.path.join(cf.pp_data_path,'{}_img.npy'.format(pid)) for pid in pids]
    segs = [os.path.join(cf.pp_data_path,'{}_msk.npy'.format(pid)) for pid in pids]

    data = OrderedDict()
    for ix, pid in enumerate(pids):
        data[pid] = {'data': imgs[ix], 'seg': segs[ix], 'pid': pid, 'raw_pid' : raw_pids[ix], 'class_target': class_targets[ix]}
...
class BatchGenerator(SlimDataLoaderBase):
    ...
    def generate_train_batch(self):
        ...
        for b in batch_ixs:
            ...
            if np.sum(seg) > 0:
                class_targets = [0 for _ in patient['class_target']]
            else:
                class_targets = [-1]
...
class PatientBatchIterator(SlimDataLoaderBase):
    ...
    def __init__(self, data, cf): #threads in augmenter
        ...
        self.dataset_raw_pids = [v['raw_pid'] for (k, v) in data.items()]
        self.patch_size = cf.patch_size
        if len(self.patch_size) == 2:
            self.patch_size = self.patch_size + [1]


    def preprocess_patient(self, img_path, msk_path):
        # x = sitk.ReadImage(os.path.join(path, 'lsa_ct.nii.gz'))
        # p = sitk.ReadImage(os.path.join(path, 'lsa_pet.nii.gz'))
        x = sitk.ReadImage('/home/cg/medicaldetectiontoolkit/tooth_dataset/test/img/1000813648_20180116.nii.gz')
        p = sitk.ReadImage('/home/cg/medicaldetectiontoolkit/tooth_dataset/test/label/1000813648_20180116.nii.gz')
        ...
        data = np.concatenate((x[None], p[None])).astype(np.float32)
        return data
    ...
    def generate_train_batch(self):
        ...
        raw_pid = self.dataset_raw_pids[self.patient_ix]        
        patient = self._data[pid]
        print('generate_train_batch, pid = ', pid, ', row_pid = ', raw_pid)     

        data = np.transpose(np.load(patient['data'], mmap_mode='r'), axes=(1, 2, 0))[np.newaxis] # do not modify!!! (c, y, x, z) (1, 400, 400, 280)
        seg = np.transpose(np.load(patient['seg'], mmap_mode='r'), axes=(1, 2, 0)) # do not modify!!!
        # print('ori_data =', data.shape) # c, h, w (z, y, x)
        # print('ori_seg =', seg.shape)  # c, h, w (z, y, x)
        ...
        # print('pad_data =', data.shape)
        # print('pad_seg =', seg.shape)
        ...
        if self.cf.dim == 3 or self.cf.merge_2D_to_3D_preds:
            out_data = data[np.newaxis] # do not modify!!! (1, 400, 400, 280) -> # (1, 1, 400, 400, 280)
            out_data = out_data[:,:,:,:,:]
            out_seg = seg[np.newaxis, np.newaxis] # do not modify!!!
            out_targets = batch_class_targets           
            # print('out_data =', out_data.shape)
            # print('out_seg =', out_seg.shape)
            
            # cut to do ConvertSegToBoundingBoxCoordinates()
            stride = 50
            slice_num = data.shape[-1] # (1, 400, 400, 280) -> 280
            cut_num = math.ceil(slice_num/stride) # (1, 400, 400, 280) -> 280/100 = 3
            ...
            for i in range(1, cut_num + 1):
                ...
                tmp_data = out_data[:, :, :, :, start:end]
                tmp_seg = out_seg[:, :, :, :, start:end]
                print('tmp_data =', tmp_data.shape)
                # print('tmp_seg =', tmp_seg.shape)
                ...
                batch_3D = {'data': tmp_data, 'seg': tmp_seg, 'class_target': out_targets, 'pid': pid}
                batch_3D = converter(**batch_3D)
                print('converter_ok')
                print('bb_targets =', batch_3D['bb_target'].shape)
                print('roi_labels =', batch_3D['roi_labels'].shape)

                # ignore the first one to concatenate
                if len(batch_3D['bb_target'].shape) == 3:
                    if not is_first:
                        bb_targets = batch_3D['bb_target']
                        roi_labels = batch_3D['roi_labels']
                        is_first = True
                    else:
                        bb_targets = np.concatenate((bb_targets, batch_3D['bb_target']), axis=1) # (1, 13, 6)
                        roi_labels = np.concatenate((roi_labels, batch_3D['roi_labels']), axis=1) # (1, 13)
                        print('new bb_targets =', bb_targets.shape)
                        print('new roi_labels =', roi_labels.shape)
                
                start = start + stride

            # print('! bb_targets =', bb_targets.shape)
            # print('! roi_labels =', roi_labels.shape)
            # print(len(np.unique(bb_targets[0])))

            batch_3D = {'data': out_data, 'seg': out_seg, 'class_target': out_targets, 'pid': pid}
            batch_3D.update({'patient_bb_target': bb_targets,
                                  'patient_roi_labels': roi_labels,
                                  'original_img_shape': out_data.shape})
            # print(batch_3D['data'].shape)

        out_batch = batch_3D if self.cf.dim == 3 else batch_2D
        patient_batch = out_batch
        # print(batch_3D['patient_bb_target'].shape)
        # print(batch_3D['patient_roi_labels'].shape)
        ...
        if np.any([data.shape[dim + 1] > self.patch_size[dim] for dim in range(3)]):
            ...
            # print('data =', data.shape) # c, h, w (z, y, x)
            # print('seg =', seg.shape)  # c, h, w (z, y, x)
